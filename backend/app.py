...
@cross_origin()
def create_images_api():
    json_data = request.get_json(force=True)
    text_prompt = json_data["text"]
    num_images = json_data["num_images"]
    steps = json_data["steps"]
    stable_diff_model = StableDiffusionWrapper(json_data["model"])
...
@cross_origin()
def enhance_images_api():
    json_data = request.get_json(force=True)
    text_prompt = json_data["text"]
    img = json_data["img"]
    steps = json_data["steps"]
    stable_diff_model = StableDiffusionWrapper("stabilityai/stable-diffusion-x4-upscaler")
    generated_imgs = stable_diff_model.enhance_image(text_prompt, image, steps)
